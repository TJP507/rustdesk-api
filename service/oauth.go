...
func (os *OauthService) GetOauthConfig(op string) (err error, oauthInfo *model.Oauth, oauthConfig *oauth2.Config, provider *oidc.Provider) {
	...
	//case model.OauthTypeGoogle: // Google is handled separately to avoid an extra FetchOidcEndpoint request
...
func (os *OauthService) callbackBase(oauthConfig *oauth2.Config, provider *oidc.Provider, code string, verifier string, nonce string, userData interface{}) (err error, client *http.Client) {
	...
	// Retrieve the ID Token; GitHub does not provide an id_token
...
// linuxdoCallback handles the linux.do OAuth callback
...
// oidcCallback handles the OIDC callback and retrieves user information via the authorization code
...
// InfoById retrieves OAuth information by ID
...
// InfoByOp retrieves OAuth information by provider name
...
// GetTypeByOp retrieves the OAuth type by provider name
...
// ValidateOauthProvider validates that an OAuth provider is correctly configured
...
// IsOauthProviderExist checks whether an OAuth provider exists
func (os *OauthService) IsOauthProviderExist(op string) bool {
	oauthInfo := &model.Oauth{}
	// Use GORM's Take method to find a matching record
...
// GetOauthProviders retrieves all configured OAuth providers
